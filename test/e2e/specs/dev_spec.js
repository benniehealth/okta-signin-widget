...
describe('Dev Mode flows', function () {
  ...
  it('can login and receive tokens on a callback using the showSignAndRedirect method', function () {
    var options = {
      clientId: '{{{WIDGET_CLIENT_ID}}}',
      redirectUri: 'http://localhost:3000/done',
      scopes: ['openid', 'profile']
    ...
    browser.executeScript(`oktaSignIn.showSignAndRedirect(${JSON.stringify(options)})`);
